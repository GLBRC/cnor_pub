...
(cNOR_delta_SI <- cNOR %>%
  mutate(
    ...
    delta_o =
      paste(round(as.numeric(cNOR$delta_o), 1)),
    
    delta_p =
      paste(round(as.numeric(cNOR$delta_p), 1)),
    
    delta_alpha =
      paste(round(as.numeric(cNOR$delta_alpha), 1)),
    
    delta_beta =
      paste(round(as.numeric(cNOR$delta_beta), 1)),
    
    SP =
      paste(round(as.numeric(cNOR$SP), 1))    
  ) %>%
...
(exp_Rayleigh_SP <- ggplot(data = cNOR, mapping = 
         aes(x = `-flnf/(1-f)`))+
  geom_point(mapping = aes(y = SP), color = "#0072B2",
             shape = 16)+
  scale_x_continuous(limits = c(0, 1), breaks = c(0.25*0:5),
                     labels = paste0(c(0.25*0:5),""))+    
  ylim(0, 10)+
...
#Figure 1 for MS version 2: Plot N2O produced vs time for cNOR and no-enzyme controls (GC-ECD data)
```{r plot_Fig_1_v2}

#Combine experimental cNOR data and no-enzyme control data
N2O_prod <- cNOR %>%
  #Select the columns in cNOR that match no_enzyme
  filter(!Name == "time_0") %>%
  dplyr::select(Date, Name, Replicate, Time_min, N2O_nmol, N_s0) %>%
  mutate(
    N2O_umol = N2O_nmol/1000
  ) %>%
  dplyr::select(1:5, N2O_umol, everything()) %>%
  dplyr::select(-N2O_nmol) %>%
  
  #Combine tables
  rbind(no_enzyme) %>%
  
  #Name replicates for legend
  mutate(
  Rep_name = c(
    rep("cNOR Replicates 1-3", each = 13),
    rep("No Enzyme Replicates 1-2", each = 4)
  )
)

(N2O_prod_fig <- ggplot()+
  geom_point(data = N2O_prod,
    mapping = 
         aes(x = Time_min, y = N2O_umol, shape = Rep_name, color = Rep_name))+
    
  scale_color_manual(name = "Rep_name",
    values = c("#CC79A7", "black"),
    labels = c(
    expression(paste(bold("cNOR"))),
    expression(paste(bold("No Enzyme")))
  ))+
 
  scale_shape_manual(name = "Rep_name",
    values = c(17, 16),
    labels = c(
    expression(paste(bold("cNOR"))),      
    expression(paste(bold("No Enzyme")))
...
ggsave(N2O_prod_fig, filename = here("graphs/exp_cNOR/N2O_prod_fig.tiff"), height = 3, width = 5, units = "in")
