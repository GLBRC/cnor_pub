...
#Figure S7: Plot N2O produced vs time for no-enzyme controls (GC-ECD data)
```{r plot_Fig_S7}

no_enzyme <- no_enzyme %>%
  mutate(
  Rep_name = c(
    rep("Replicate 1", each = 2),
    rep("Replicate 2", each = 2)
  )
)

(no_enzyme_fig <- ggplot()+
  geom_point(data = no_enzyme,
    mapping = 
  ...
  scale_color_manual(name = "Replicate",
    ...
    labels = c(
    expression(paste(bold("Replicate 1"))),
    expression(paste(bold("Replicate 2")))
  ))+
...
ggsave(no_enzyme_fig, filename = here("graphs/exp_controls/no_enzyme_fig.tiff"), height = 3, width = 5, units = "in")
...
(controls_delta_SI <- controls %>%
  mutate(
    ...
    delta_o =
      paste(round(as.numeric(controls$delta_o), 2)),
    
    delta_p =
      paste(round(as.numeric(controls$delta_p), 2)),
    
    delta_alpha =
      paste(round(as.numeric(controls$delta_alpha), 2)),
    
    delta_beta =
      paste(round(as.numeric(controls$delta_beta), 2)),
    
    SP =
      paste(round(as.numeric(controls$SP), 2))    
  ) %>%
